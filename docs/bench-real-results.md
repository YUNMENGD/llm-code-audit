...
### flask（5 条，T0/F3/?2）——特殊数据点：治理链**全开后**的残留基线

flask 治理前逐行清单未完整留档（探针实验只有聚合数 8→6），所以它的 5 条直接以当前
扫描为基线：**removed 恒 0，precision=0.0（静态口径）**。价值不在降噪统计，而是回答
"确定性豁免打光之后，静态层在成熟框架上还剩什么"——答案是 3 条 A 模式（from_pyfile
的 exec 是文档承诺机制、CLI 兜底 traceback 是刻意设计、loads 降级 pass 有注释自证

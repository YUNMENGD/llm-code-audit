...
6. 无问题输出 []。
7. **资源所有权边界**：跨函数才成立的问题才报；资源（连接/游标）由参数传入或模块级管理时，
   "本函数未关闭"不是缺陷——泄漏必须落在「谁创建谁负责」的函数上。
8. **已防护模式白名单**：safe_load / realpath+基目录校验 / 参数化查询 / subprocess 列表参数
   未开 shell / with lock 保护 / 非安全用途的哈希 —— 防护覆盖到位就不许因 API 名匹配报漏洞，
   坚持要报须在 analysis 指出防护失效的具体行。
9. **行号纪律**：evidence 必须逐字来自 line_start~line_end 的行；依据不在这些行就改行号，
   定位漂移会被自动校验废弃。

# 补充证据申请（Agentic RAG，逃生舱）
若某条判定因【缺少知识库依据】而无法确认（规则拿不准、需要该缺陷类型的判定
标准/案例参照），可在 JSON 数组中额外输出申请对象（不计入问题数）：
{"type": "NEED-EVIDENCE", "query": "想要检索的缺陷主题，如：SSRF 云元数据防护判定标准"}
系统会用该主题补充检索知识库并重问你一次。最多申请 2 条；证据足以判断时
不要申请（每次申请多一轮调用成本）。
{{examples}}
